...
namespace Tn
{
	...
	using nvinfer1::TinyYoloLayerPlugin;
	class PluginFactory : public nvinfer1::IPluginFactory, public nvcaffeparser1::IPluginFactoryExt
	{
		public:
			...
			inline bool isTinyYolo(const char* layerName)
			{
				return strcmp(layerName,"tiny-yolo-det") == 0;
			}
			...
			virtual nvinfer1::IPlugin* createPlugin(const char* layerName, const nvinfer1::Weights* weights, int nbWeights) override
			{
				...
				else if (isTinyYolo(layerName))
				{
					assert(nbWeights == 0 && weights == nullptr && mPluginTinyYolo.get() ==  nullptr);
					mPluginTinyYolo.reset(new TinyYoloLayerPlugin(CUDA_THREAD_NUM));
					return mPluginTinyYolo.get();
				}
				else
				{
				assert(0);
		...
		nvinfer1::IPlugin* createPlugin(const char* layerName, const void* serialData, size_t serialLength) override
		{
			...
			{
				assert(mPluginTinyYolo.get() ==  nullptr);
				mPluginTinyYolo.reset(new TinyYoloLayerPlugin(serialData, serialLength));
				return mPluginTinyYolo.get();
			}
			else
			{
		...
		bool isPluginExt(const char* name) override
		{
			...
			return isLeakyRelu(name) || isUpsample(name) || isYolo(name) || isTinyYolo(name); 
		}
		...
		std::unique_ptr<TinyYoloLayerPlugin> mPluginTinyYolo {nullptr};
