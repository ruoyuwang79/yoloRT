...
{
	list<string> 
	readFileList(const string& fileName)
	{
	...
	list<Source> 
	readLabelFileList(const string& fileName)
	{
		ifstream file(fileName);  
		if(!file.is_open())
		{
	...
	tuple<list<string>, list<vector<Bbox>>> 
	readObjectLabelFileList(const string &fileName)
	{
